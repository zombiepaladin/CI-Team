private var fWheelRadius : float = 1.8796;
private var rWheelRadius : float = 1.6256;
...
function SetupWheel(wheelTransform : Transform, isFrontWheel : boolean)
{
	...
	if (isFrontWheel)
	{
		...
		fWheelRadius = wheel.tireGraphic.renderer.bounds.size.y / 2;	
		wheel.collider.radius = fWheelRadius;
		...
		rWheelRadius = wheel.tireGraphic.renderer.bounds.size.y / 2;	
		wheel.collider.radius = rWheelRadius;
		}
	return wheel;
}
...
function UpdateWheelGraphics(relativeVelocity : Vector3)
{
	...
	for(var w : Wheel in wheels)
	{
		...
		{
			if(w.steerWheel){
				w.wheelGraphic.localPosition = wheel.transform.up * (fWheelRadius + wheel.transform.InverseTransformPoint(wh.point).y);
				w.wheelVelo = rigidbody.GetPointVelocity(wh.point);
				w.groundSpeed = w.wheelGraphic.InverseTransformDirection(w.wheelVelo);
			}
			else{
				w.wheelGraphic.localPosition = wheel.transform.up * (rWheelRadius + wheel.transform.InverseTransformPoint(wh.point).y);
				w.wheelVelo = rigidbody.GetPointVelocity(wh.point);
				w.groundSpeed = w.wheelGraphic.InverseTransformDirection(w.wheelVelo);
			}
		}
		...
		if(w.steerWheel)
		{
			...
			w.tireGraphic.Rotate(Vector3.right * (w.groundSpeed.z / fWheelRadius) * Time.deltaTime * Mathf.Rad2Deg);
		}
		else if(!handbrake && w.driveWheel)
		{
			...
			w.tireGraphic.Rotate(Vector3.right * (w.groundSpeed.z / rWheelRadius) * Time.deltaTime * Mathf.Rad2Deg);
